...
def caluclate_profit(x, y_pred, y_real, include_winloss=False, include_ml=True, include_spread=True, include_ou=True):
    ...
    for i in range(0, len(x)):
        ...
        if include_ml:
            if y_pred[i][0] > y_pred[i][1] and y_real[i][0] > y_real[i][1]: # ML visitor win
                ...
                ml_profit -= 1
                ml_record[1] += 1
                if y_real[i][0] > y_real[i][1] and y_pred[i][0] < y_pred[i][1] and x[i][hml_i] < 0: # we bet home underdog and lost
                    bet_rate += 1
                elif y_pred[i][1] > y_pred[i][0] and y_pred[i][1] < y_pred[i][0] and x[i][vml_i] < 0: # we bet away underdog and lost
                    bet_rate += 1
            else: # push
                ml_record[2] += 1
    ...
    return ml_record, spread_record, ou_record, ml_profit, spread_profit, ou_profit